  ...
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/138800db-6a5c-4bf8-b2f2-bafbde22178d",
        {
          method: "PATCH",
        },
      );
    ...
    test("With valid token", async () => {
      ...
      expect(uuidVesion(responseBody.user_id)).toBe(4);
    ...
    test("With valid token and already activated user", async () => {
      const createdUser = await orchestrator.createUser();
